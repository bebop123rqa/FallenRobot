...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nᴛʜɪs ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟ ᴏғ ᴛʜɪs ᴜsᴇʀ ɪs <b>Dev</b>.\n"
        disaster_level_present = True
    ...
    if disaster_level_present:
        text += ' \n[<a href="https://t.me/Chika_Support/440">ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴋɴᴏᴡ ᴡʜᴀᴛ ɪs ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟs.</a>]'.format(
            bot.username
        )
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Umm... yeah, I only trust Karasuna Association to set my bio."
            )
            return
